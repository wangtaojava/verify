...
//	private static String myqyEmail = "wangtao@easypay.com.cn";
	...
	public static void send(String toAddress, String path, String mername) {
		try {
			logger.info("===邮件开始发送===");
			/*
...
//			addressesCc[0] = new InternetAddress(myqyEmail);
			...
			message.setRecipients(Message.RecipientType.CC, addressesCc);// 抄送
			message.setSubject(mername + "测试参数");
			// 设置邮件消息内容、包含附件
			...
			// 设置正文内容
			body.setContent("您好，商户“" + mername + "”的测试参数已在附件中，请查收。", "text/html;charset=utf-8");
			// 设置附件内容
			attach.setDataHandler(new DataHandler(new FileDataSource(path)));
			attach.setFileName(MimeUtility.encodeText(mername + "测试参数.txt"));
...
//		sendMail();
//		send("1392148563@qq.com", "C:\\Users\\pll\\Desktop\\company.xls");
